...
const Loader = () => {
    ...
    return(<div className="z-50 fixed top-0 left-0 h-screen w-screen">
        ...
        <div className="z-50 absolute bwp-bg-white w-40 h-40 p-5 rounded-full shadow inset-1/2 -mt-20 -ml-20">
            ...
            <div className="flex justify-center align-middle mx-auto -mt-12">
                { t("lb_loading").split("").map((s, i)=> {
                    return (<div className={`font-semibold text-2xl animate-bounce px-1 ${ i%2 === 0 ? "animate-delay-100" : ""}`}>{s}</div>);
                })}
            </div>
        </div>
    </div>)
